  ...
  document.getElementById('submitProposal').addEventListener('click', async () => {
    ...
    const emailHtml = `
      <div style="font-family: Arial, sans-serif; color: #4a2d3f; line-height: 1.6;">
        ...
        ${val ? `<p><strong>Date proposée 🗓️:</strong> ${pretty}</p>` : '<p><em>Aucune date proposée</em></p>'}
        ${proposalCalendarLinks ? `<p><a href="${proposalCalendarLinks.httpsUrl}">Ajouter au calendrier Apple</a></p>` : ''}
    ...
    try {
      const resp = await postEmail({
        ...
        secondaryEmail: {
          ...
          html: `<div style="font-family: Arial, sans-serif; line-height: 1.6; color: #222;"><p>💡 <strong>Nouvelle proposition d'activité</strong></p>${title ? `<p><strong>Titre 😚:</strong> ${title}</p>` : ''}${details ? `<p><strong>Détails 🫡:</strong> ${details.replace(/\n/g, '<br/>')}</p>` : ''}${val ? `<p><strong>Date proposée 🗓️:</strong> ${pretty}</p>` : ''}${proposalCalendarLinks ? `<p><a href="${proposalCalendarLinks.httpsUrl}">Ajouter au calendrier Apple</a></p>` : ''}<p>✨J'espère qu'il t'offrira une très belle expérience et un moment précieux ensemble !</p></div>`,
        },
    ...
    document.getElementById('confirmSchedule').addEventListener('click', async () => {
      ...
      const emailHtml = `
        <div style="font-family: Arial, sans-serif; color: #4a2d3f; line-height: 1.6;">
          ...
          ${activityCalendarLinks ? `<p><a href="${activityCalendarLinks.httpsUrl}">Ajouter au calendrier Apple</a></p>` : ''}
          <p>✨ J'espère que tu vas lui offrir une très belle expérience et un moment précieux ensemble !</p>
        </div>
      `;
